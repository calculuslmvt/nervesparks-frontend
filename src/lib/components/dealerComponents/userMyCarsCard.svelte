<script lang="ts">
    ...
    const handleSoldVehicle = async()=> {
        const url = serverUrl + '/dealership/sold-car-info';
    ...
    const handleClick = async(deal)=> {
        ...
        const url = serverUrl + "/dealership/remove-deal"; 
        const response = await axios.post(url, dealId, {withCredentials: true})
        console.log(response); 
